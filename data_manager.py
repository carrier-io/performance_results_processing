...
class DataManager():
    ...
    def compare_with_thresholds(self, test, test_data, add_green=False):
        ...
        def compile_violation(request, th, total_checked, total_violated, compare_with_thresholds, add_green=False):
            # TODO here we should check deviations
            total_checked += 1
        ...
        def compile_globals(request, th, compare_with_globaly_applicable):
            # TODO here we should check deviations
            color, metric = self._compare_request_and_threhold(request, th)
            if color != "green":
    ...
    def compare_with_baseline_summary(self, baseline_summary, current_test_summary, quality_gate_config):
        compare_with_baseline_summary_results = []
        if quality_gate_config.get("settings", {}).get("summary_results", {}).get("check_error_rate", False):
            ...
            _res = {"type": "Baseline error rate", "status": "Failed", "message": f"Error rate for current test results - {current_test_error_rate}% is higher than error rate for baseline test - {baseline_error_rate}%"} if error_rate_div > div_value  else\
                {"type": "Baseline error rate", "status": "Success", "message": "Error rate for current test results is less or equal to error rate for baseline test"}
            compare_with_baseline_summary_results.append(_res)
        if quality_gate_config.get("settings", {}).get("summary_results", {}).get("check_throughput", False):
            ...
            _res = {"type": "Baseline throughput", "status": "Failed", "message": f"Throughput for current test results - {current_test_summary['throughput']} req/sec is less baseline - {baseline_summary['throughput']} req/sec"} if throughput_div > div_value  else\
                {"type": "Baseline throughput", "status": "Success", "message": "Throughput for current test results is higher or equal to throughput for baseline test"}
            print(_res)
            compare_with_baseline_summary_results.append(_res)

        comparison_metric = quality_gate_config["baseline"].get('rt_baseline_comparison_metric', 'pct95')

        if quality_gate_config.get("settings", {}).get("summary_results", {}).get("check_response_time", False):
            ...
            _res = {"type": "Baseline response time", "status": "Failed",
                    "message": f"Response time for current test results by {comparison_metric} - {current_test_summary[comparison_metric]} ms is higher than response time for baseline test - {baseline_summary[comparison_metric]} ms"} if response_time_div > div_value else \
                {"type": "Baseline response time", "status": "Success",
    ...
    def compare_with_baseline_per_request(self, baseline, current_test_results, quality_gate_config):
        ...
        if failed_requests_rate > qg_failed_requests_rate:
            compare_with_baseline_per_request_results.append({"type": "Baseline compare per request", "status": "Failed",
                    "message": f"Percentage of failed requests compare to baseline is more than {qg_failed_requests_rate}%"})
        else:
            compare_with_baseline_per_request_results.append({"type": "Baseline compare per request", "status": "Success",
                                                              "message": f"Percentage of failed requests compare to baseline is less than {qg_failed_requests_rate}%"})


        return compare_with_baseline_per_request_results
